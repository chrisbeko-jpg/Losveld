...
export default function Login(){
 ...
 useEffect(()=>{const params=new URLSearchParams(window.location.search);if(params.get('role')==='host'){setMode('signup');setRole('host')}},[]);
 ...
 return <main><header className="nav wrap"><Link className="logo" href="/">LOS<span>!</span></Link></header><section className="authWrap"><div className="authCard wide"><span className="kicker">MIJN LOS!</span><h1>{mode==='login'?'Welkom terug':'Wat wil je met LOS! doen?'}</h1><p>{mode==='login'?'Log in voor je boekingen, honden en verhuurdersdashboard.':'Kies hoe je wilt starten. Een verhuurder kan later ook gewoon zelf een veld boeken.'}</p>{mode==='signup'&&<div className="roleCards"><button type="button" className={role==='guest'?'roleCard active':'roleCard'} onClick={()=>setRole('guest')}><Dog/><div><strong>Ik wil een LOS! boeken</strong><span>Voor hondeneigenaren die privé willen rennen, trainen of spelen.</span></div>{role==='guest'&&<ArrowRight/>}</button><button type="button" className={role==='host'?'roleCard active':'roleCard'} onClick={()=>setRole('host')}><Fence/><div><strong>Ik wil mijn veld verhuren</strong><span>Voor grondeigenaren die hun terrein beschikbaar willen stellen.</span></div>{role==='host'&&<ArrowRight/>}</button></div>}<form onSubmit={submit}>{mode==='signup'&&<label>Naam<input value={name} onChange={e=>setName(e.target.value)} required/></label>}<label>E-mailadres<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required/></label><label>Wachtwoord<input type="password" minLength="6" value={password} onChange={e=>setPassword(e.target.value)} required/></label><button className="bookNow" disabled={busy}>{busy?'Bezig...':mode==='login'?'Inloggen':role==='host'?'Verhuurdersaccount maken':'Account maken'}</button></form>{msg&&<div className="authMessage">{msg}</div>}<button className="authSwitch" onClick={()=>setMode(mode==='login'?'signup':'login')}>{mode==='login'?'Nog geen account? Kies hoe je wilt starten':'Al een account? Inloggen'}</button></div></section></main>
}